...
func (r Running) Calories() float64 {
	...
	return ((CaloriesMeanSpeedMultiplier*meanSpeed + CaloriesMeanSpeedShift) * weight / MInKm * float64(duration) * MinInHours)
}
...
func (w Walking) Calories() float64 {
	...
	meanSpeedSecond := w.Training.meanSpeed() / 3600
	height := w.Height
	durationHour := w.Training.Duration.Hours()

	return ((CaloriesWeightMultiplier*weight + (math.Pow(meanSpeedSecond, 2)/height/100)*CaloriesSpeedHeightMultiplier*weight) * durationHour * MinInHours)
}
